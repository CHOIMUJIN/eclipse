...
public class ChattingClient {
	...
	// 서버와 송/수신하기 위한 클라이언트 소켓
	private Socket socket;
	// 서버에서 문자를 수신하는 스트림
	private BufferedReader in;
	// 서버에 문자를 송신하는 스트림
	private PrintWriter out;
	// 함수형 인터페이스 Consumer : 메소드의 입력데이터(파라미터들)를 받아서 기능을 수행하고 반환하지 않음
	...
	// 서버에서 클라이언트에 전송하는 메세지르르 수신하는 Thread
		...
		new Thread(
			() -> {
				try {
					...
					String line;
					// 메세지(line)가 있으면 line을 onMesssageReceived라는 Consumer에 전달
